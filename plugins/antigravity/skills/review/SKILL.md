...
2. **Dispatch each reviewer it names**, giving it the `base..HEAD` range,
   ...
   Antigravity bundles both reviewers with this plugin, under `agents/`,
   so `invoke_subagent` can reach them by name once the plugin is
   enabled. If neither resolves there is nothing to dispatch, and the
   honest answer is to say so -- not to review the change yourself,
   which is the one thing this skill exists to prevent.
3. **Read the verdict out of the reviewer's own final message.**

   On Claude Code and Codex a `SubagentStop` hook captures that message
   and `canon_review` reads it back, so the verdict reaching `ship` is
   the reviewer's own words rather than this session's retelling.
   **Antigravity fires no such event**, so that capture does not happen
   here and `canon_review` will report no stored verdict. Quote the
   reviewer's closing verdict line verbatim instead of paraphrasing it,
   and say that it was read in-session. Do not present a paraphrase as
   though it came back through `canon_review`: a weakened guarantee
   stated plainly is worth more than a strong one that isn't true.
   See this plugin's README for the standing limitation.
