...
"""Canon's `PostToolUse:Edit|Write` hook -- scope departure.
...
populated -- `save_plan.py` always writes `scope:` blank by design (see
its own module docstring), meant to be filled in later by a human or
the `plan` skill's own dialogue -- so this hook has nothing to compare
...
def main() -> None:
    ...
    if payload is None:
        return
    if payload.get("tool_name") not in (None, "Edit", "Write"):
        return
